...
contract MultiSigWallet {
    ...
    function submitBatch(address destination, uint value, bytes32[] data, uint nonce)
        external
        ...
        returns (bool success)
    {
        ...
        for(uint i=0; i < data.length; i++){
            n++;
            transactionHash = sha3(destination, value, data[i], n);
    ...
    function confirmBatch(bytes32[] transactionHashs)
        public
        ownerExists(msg.sender)
        //notExecuted(transactionHash)
    {
        for(uint i=0; i<transactionHashs.length; i++){
            confirmations[transactionHashs[i]][msg.sender] = true;
            Confirmation(msg.sender, transactionHashs[i]);
            if (confirmationCount(transactionHashs[i]) >= required) {
                Transaction tx = transactions[transactionHashs[i]];
                tx.executed = true;
                if (!tx.destination.call.value(tx.value)(tx.data))
                    throw;
                Execution(msg.sender, transactionHashs[i]);
            }       
        }
    }
